...
<?php
	...
	{
		$db->query("update `al` set `content`='$content',`title`='$title',`picurl`='$picurl',`sorts`='$sorts',`en_content`='$en_content' where `id`='$ceid' limit 1");
		Show_msg("al_manage.php","修改成功！");
	}
...
{
	$cid=$_REQUEST["cid"];
	$query2=($db->query("select * from `al` where `id`='$cid' limit 1"));
...
<table border=0 cellspacing=1 align=center class=form>
  ...
  <td><input name="en_content" type="text" id="en_content" value="<?php echo $row2["en_content"]?>" size="70"></td>
  </tr>
...
<table border=0 cellspacing=1 align=center class=form>
    ...
    <form name="form1" method="post" action="al_manage.php?f_id=<? echo $f_id ?>">
		...
		<?php
			...
			$result=$db->query("select `id` from `al`");
		$num=$db->num_rows($result);
		pageft($num, 20,"?txtsea=$search");
		if ($firstcount < 0) $firstcount = 0;
		$query=$db->query("select * from `al` order by sorts desc,id desc limit $firstcount, $displaypg");

<?php include_once("inc/config.php");
	...
	if(!empty($_REQUEST["cdid"]))//删除单条记录
	{
		...
		$f_id=$_REQUEST["f_id"];
		Deldy("hc",$cdid);
		Show_msg("hc_manage.php","删除成功！");
	}
	if(isset($_REQUEST["delall"]))//批量删除记录
	{
		$f_id=$_REQUEST["f_id"];
		Delall("hc","id",$_REQUEST["SoftID"]);
		Show_msg("hc_manage.php","删除成功!");
	}	
	...
	if($_REQUEST["act"]=="eds")//修改排序值
	{
		...
		$id=$_REQUEST["id"];
		$f_id=$_REQUEST["f_id"];
		$db->query("update `hc` set `sorts`='$sorts' where `id`='$id' limit 1");
		Show_msg("hc_manage.php","修改成功!");
	}
	?>
...
{
	window.location="hc_manage.php?act=eds&f_id="+f_id+"&tb=al&id="+id+"&sorts="+document.getElementById("sorts"+id).value;
}
...
<?php
	...
	{
		$db->query("update `hc` set `title`='$title',`jianjie`='$jianjie',`picurl`='$picurl',`sorts`='$sorts' where `id`='$ceid' limit 1");
		Show_msg("hc_manage.php","修改成功！");
	}
...
{
	$cid=$_REQUEST["cid"];
	$query2=($db->query("select * from `hc` where `id`='$cid' limit 1"));
	$row2=$db->fetch_array($query2);	
?>
<form name="form2" method="post" action="hc_manage.php">
<table border=0 cellspacing=1 align=center class=form>
  ...
  <td width="86%"><input name="title" type="text" id="title" size="40" value="<?php echo $row2["title"]?>"/></td>
  </tr>
  <tr>
 ...
 <tr>
       	    ...
       	    <td><textarea name="jianjie" rows="10" id="jianjie" style="width:70%"><?php echo $row2["jianjie"]?></textarea></td>
    </tr>
       	  <tr>
...
<table border=0 cellspacing=1 align=center class=form>
    ...
    $f_id=$_REQUEST["f_id"];
	?>
    <form name="form1" method="post" action="hc_manage.php?f_id=<? echo $f_id ?>">
     	  <tr>
		...
		<?php
			...
			$result=$db->query("select `id` from `hc`");
		$num=$db->num_rows($result);
		pageft($num, 20,"?txtsea=$search");
		if ($firstcount < 0) $firstcount = 0;
		$query=$db->query("select * from `hc` order by sorts desc,id desc limit $firstcount, $displaypg");
